...
describe Webdrivers::MSWebdriver do
  ...
  describe '#update' do
    ...
    it 'downloads 32 bit msedgedriver directly' do
      allow(mswebdriver).to receive(:downloads).and_return(mswebdriver.send(:downloads))
      allow(mswebdriver).to receive(:windows_version).and_return(Gem::Version.new(9999))
      allow(Selenium::WebDriver::Platform).to receive(:bitsize).and_return(32)

      mswebdriver.update

      expect(mswebdriver).not_to have_received(:downloads)
      expect(File.exist?(mswebdriver.binary)).to eq true
    end

    it 'downloads 64 bit msedgedriver directly' do
      allow(mswebdriver).to receive(:downloads).and_return(mswebdriver.send(:downloads))
      allow(mswebdriver).to receive(:windows_version).and_return(Gem::Version.new(9999))
      allow(Selenium::WebDriver::Platform).to receive(:bitsize).and_return(64)
